...
public class CreateValueSetConceptMetadataValue
{
  ...
  public CreateValueSetConceptMetadataValueResponseType CreateValueSetConceptMetadataValue(CreateValueSetConceptMetadataValueRequestType parameter, String ipAddress)
  {

    return CreateValueSetConceptMetadataValue(parameter, null, ipAddress);
  }

  public CreateValueSetConceptMetadataValueResponseType CreateValueSetConceptMetadataValue(CreateValueSetConceptMetadataValueRequestType parameter, org.hibernate.Session session, String ipAddress)
  {
    if (logger.isInfoEnabled())
      logger.info("====== CreateValueSetConceptMetadataValue gestartet ======");

    boolean createHibernateSession = (session == null);
    logger.debug("createHibernateSession: " + createHibernateSession);

    // Return-Informationen anlegen
    CreateValueSetConceptMetadataValueResponseType response = new CreateValueSetConceptMetadataValueResponseType();
    ...
    try
    {
      ...
      {   // 1. try-catch-Block zum Abfangen von Hibernate-Fehlern        
        ...
        response.getReturnInfos().setMessage("ValueSetConceptMetadataValue erfolgreich erstellt");
      }
      catch (Exception e)
      {
        ...
        response.getReturnInfos().setStatus(ReturnType.Status.FAILURE);
        response.getReturnInfos().setMessage("Fehler bei 'CreateValueSetConceptMetadataValue', Hibernate: " + e.getLocalizedMessage());
    ...
    catch (Exception e)
    {
      ...
      response.getReturnInfos().setOverallErrorCategory(ReturnType.OverallErrorCategory.ERROR);
      response.getReturnInfos().setStatus(ReturnType.Status.FAILURE);
      response.getReturnInfos().setMessage("Fehler bei 'CreateValueSetConceptMetadataValue': " + e.getLocalizedMessage());// + " ; Mögliche Ursache: Verwendete Id(" + Long.toString(parameter.getValueSet().getId()) +  ") nicht in Datenbank.");           
      logger.error(response.getReturnInfos().getMessage());
      e.printStackTrace();
    }
    return response;
   ...
   */
  private boolean validateParameter(CreateValueSetConceptMetadataValueRequestType Request, CreateValueSetConceptMetadataValueResponseType Response)
  {
    boolean erfolg = true;
