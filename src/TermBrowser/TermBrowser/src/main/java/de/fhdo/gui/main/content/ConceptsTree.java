    ...
    if (codeSystemVersionId > 0)
    {
      ...
      if (rootConceptCode == null || rootConceptCode.length() == 0)
      {
        ...
        if (response.getReturnInfos().getStatus() == Status.OK && response.getReturnInfos().getCount() > 0)
        {
          ...
          if(cseList.size() > 0)
            logger.debug("First currentVersionId: " + cseList.get(0).getCurrentVersionId());
...
//    treeConcepts.setPageSize(Integer.parseInt(paging.getPageSize()));
  ...
  private ListCodeSystemConceptsRequestType createParameterForCodeSystems(boolean onlyMainClasses)
  {
    ...
    if (paging != null)
    {
      ...
      parameter.getPagingParameter().setUserPaging(false);
      
      logger.debug("Anfrage: SetPaging: " + currentPageIndex);
