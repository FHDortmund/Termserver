...
public class ProposalDetails extends Window implements AfterCompose
{
  ...
  public void afterCompose()
  {
    ...
    {
      int selIndex = -1;
      Tabbox tb = (Tabbox) getFellow("tbAuswahl"); // Concept
      ...
      if(source instanceof CodeSystemVersion && isExisting && type.equals("")){ //Existierendes Vokabular
      ...
      if(source instanceof ValueSetVersion && isExisting && type.equals("")){ //Existierendes ValueSet
  ...
  private void initData()
  {
    ...
    Set<VocInfo> vocData = new HashSet<VocInfo>();
    Set<VocInfo> choosenVocData = new HashSet<VocInfo>();
        ...
        else if (selIndex == 1)
        {
          ...
          {  
            ...
            if(!isExisting){  
                ...
                else csvem.setIsMainClass(false);
  ...
  private boolean pruefePflichtfelder_Concept(CodeSystemConcept csc)
  {
    if (isNullOrEmtpy(csc.getCode()))
    {
      Messagebox.show("Sie müssen einen Code für das Konzept eingeben.", "Vorschlag erstellen", Messagebox.OK, Messagebox.ERROR);
      return false;
    }
    if (isNullOrEmtpy(csc.getTerm()))
    {
      Messagebox.show("Sie müssen einen Begriff für das Konzept eingeben.", "Vorschlag erstellen", Messagebox.OK, Messagebox.ERROR);
      return false;
    }
  ...
  private boolean pruefePflichtfelder_ConceptVs(ConceptValueSetMembership cvsm)
  {

    if (dlbv.getChosenDataList().isEmpty())
    {
      Messagebox.show("Sie müssen ein Code System auswählen!", "Vorschlag erstellen", Messagebox.OK, Messagebox.ERROR);
      return false;
    }
    if (((Textbox) getFellow("tbCodeVs")).getText() == null || ((Textbox) getFellow("tbCodeVs")).getText().equals(""))
    {
      Messagebox.show("Sie müssen einen gültigen Code eines Konzepts aus dem Code System angeben!", "Vorschlag erstellen", Messagebox.OK, Messagebox.ERROR);
      return false;
    }
  ...
  private boolean pruefePflichtfelder_Vocabulary(CodeSystem cs, CodeSystemVersion csv)
  {
    if (isNullOrEmtpy(cs.getName()))
    {
      Messagebox.show("Sie müssen einen Namen für das Code System eingeben.", "Vorschlag erstellen", Messagebox.OK, Messagebox.ERROR);
      return false;
    }
    if (isNullOrEmtpy(csv.getName()))
    {
      Messagebox.show("Sie müssen einen Namen für die Code System Version eingeben.", "Vorschlag erstellen", Messagebox.OK, Messagebox.ERROR);
      return false;
    }
  ...
  private boolean pruefePflichtfelder_ValueSet(ValueSet vs)
  {
    if (isNullOrEmtpy(vs.getName()))
    {
      Messagebox.show("Sie müssen einen Namen für das Value Set eingeben.", "Vorschlag erstellen", Messagebox.OK, Messagebox.ERROR);
      return false;
    }
  ...
  public boolean getSubConceptVisible()
  {
    logger.debug("getSubConceptVisible()");
    return parentTerm != null;
  }

  public boolean isSubConceptVisible()
  {
    logger.debug("isSubConceptVisible()");
    return parentTerm != null;
  }
   ...
   */
  public CodeSystemEntityVersion getParentTerm()
  {
    return parentTerm;
  }
   ...
   */
  public void setParentTerm(CodeSystemEntityVersion parentTerm)
  {
