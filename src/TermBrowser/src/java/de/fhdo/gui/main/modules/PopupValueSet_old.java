 ...
 * @author Becker
 */
public class PopupValueSet_old extends PopupWindow implements IUpdateModal
{
  public static enum EDITMODES
  {
    NONE, DETAILSONLY, CREATE, MAINTAIN_VERSION, MAINTAIN
  }
  ...
  protected void create()
  {
    ...
    if (runS)
    {
      if (cboxCSVValidityRange.getSelectedItem() != null)
      {
        ...
        try
        {
          vsv.setValidityRange(Long.parseLong(DomainHelper.getInstance().getComboboxCd(cboxCSVValidityRange)));
        }
        catch (Exception ex)
        {
        }
